...
export function ModelSelector({
  ...
  return (
    <label className="flex min-w-0 items-center gap-2 text-sm text-[var(--muted)]">
      ...
      <select
        className="min-w-0 rounded-[8px] border border-[var(--border)] bg-white/[0.05] px-2 py-1.5 text-sm text-[var(--foreground)] disabled:cursor-not-allowed disabled:opacity-60"
        disabled={disabled}
        value={value}
